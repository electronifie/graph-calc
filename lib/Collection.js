...
Collection.prototype.createOrUpdate = function (raw) {
  ...
  if (!item) {
    ...
    item.on('pre-deleted', this._removeItem.bind(this));
  } else {
    item.update(raw);
  }
  return item;
...
Collection.prototype.getAll = function () { return _.chain(this._instanceIndex).values().compact().value(); };

Collection.prototype._removeItem = function (item) { this._instanceIndex[item.id] = null; };
