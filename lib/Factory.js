...
Factory.prototype.getNode = function (nodeType, id) {
  return this.nodeCollections[nodeType].get(id);
};

Factory.prototype.getEdge = function (edgeType, id) {
  return this.edgeCollections[edgeType].get(id);
};
...
Factory.prototype.getNodes = function (nodeType) {
  if (nodeType) {
    ...
    return _.chain(this.nodeCollections).values().map(function (nf) { return nf.getAll() }).flatten().value();
  }
};
...
Factory.prototype.getEdges = function (edgeType) {
  if (edgeType) {
    ...
    return _.chain(this.edgeCollections).values().map(function (nf) { return nf.getAll() }).flatten().value();
  }
};
